...
A simple Flask web app that lets you verify whether an OpenAI API key is active and valid — without consuming any tokens.

## Features

- Paste any OpenAI API key and instantly check its status
- Uses the `models.list()` endpoint to validate the key without spending credits
...
3. The result is shown immediately:
   ...
   - ❌ **Inactive / Invalid** — authentication failed or the key lacks required permissions.
   - ⚠️ **Error** — an unexpected error occurred.

## Security note

Your API key is sent to the server only to make the validation request and is never stored or logged.